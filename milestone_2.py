...
for product_id in df['product_id'].unique():
    ...
    model_metrics.append({'model':'LSTM','RMSE':lstm_rmse,'MAPE':lstm_mape})
    model_metrics.append({'model':'ARIMA','RMSE':arima_rmse,'MAPE':arima_mape})
    model_metrics.append({'model':'prophet','RMSE':prophet_rmse,'MAPE':prophet_mape})
...
overall_performance=metrics_df.groupby('model').agg( avg_rmse=('RMSE','mean'),avg_mape=('MAPE','mean')).reset_index()
# Identify best model overall
best_overall_model = overall_performance.loc[overall_performance['avg_rmse'].idxmin(), 'model']

print("\n=== Overall Model Accuracy ===")
print(overall_performance)
print(f"\nBest Model Overall: {best_overall_model}")

# Save summary
overall_performance.to_csv("overall_model_performance.csv", index=False)
